...
def main(argv=None):
    ...
    # GPU NOTE: to use the CUDAExecutionProvider, launch via the provided
    # 'bin/i2i-server' script, which pre-sets LD_LIBRARY_PATH to the venv's
    # bundled NVIDIA libraries BEFORE exec (glibc reads LD_LIBRARY_PATH at
    # process startup; setting it here in-process would not affect dlopen).
